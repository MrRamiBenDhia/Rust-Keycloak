...
use super::csv_manager::csv_my_custom_deserialize;
use crate::{api::user::user_model::UserModel, AppState};
...
pub async fn create_new_users_from_csv(
    ...
    match post_users_request(&csv_data, &app_state).await {
        ...
        Err(err) => {
            ...
            return Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": error_message, "elapsed_time": elapsed_millis })),
            ));
        }
    }
}
...
//? My functions
pub async fn post_users_request(
    users: &[UserModel],
    data: &Arc<AppState>,
...
pub async fn delete_users_except_ids(
    ids_to_keep: Vec<String>,
    ...
    let query = format!("DELETE FROM user WHERE uid NOT IN ({})", ids_to_keep_str);
